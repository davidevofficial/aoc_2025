...
impl Lock {
    fn apply_movement(self: &mut Self, movement: Movement){
        //move by x amount either left or right and then update clicks if current_number == 0
        if movement.movementtype == MovementType::Left{
            self.current_number -= movement.amount;
        }else{self.current_number += movement.amount}

        if self.current_number < 0{
            self.current_number = (self.current_number + 1000000) % 100;
